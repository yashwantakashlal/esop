...
// Default selling caps
const DEFAULT_SELLING_CAPS = {
  MAX_AMOUNT_INR: 10000000, // 1 crore INR cap by default
  DEFAULT_PERCENT: 25 // 25% default percentage
};
 ...
 * @param {number} maxAmountInr - Maximum sellable amount in INR
 * @return {object} Object containing sellable units and the limiting factor
 */
function calculateSellableUnits(units, percentSellable, buybackPrice, maxAmountInr) {
  // 1. Calculate based on user-defined percentage
  const userDefinedUnits = Math.floor(units * (percentSellable / 100));
  
  // 2. Calculate based on monetary cap
  const monetaryCappedUnits = Math.floor(maxAmountInr / buybackPrice);
  
  // 3. Find the most restrictive constraint
  const sellableUnits = Math.min(userDefinedUnits, monetaryCappedUnits, units);
  
  // Determine the limiting factor
  let limitingFactor;
  if (sellableUnits === userDefinedUnits) {
    limitingFactor = `User-defined percentage (${percentSellable}%)`;
  } else if (sellableUnits === monetaryCappedUnits) {
    limitingFactor = `Monetary cap (${(maxAmountInr / 10000000).toFixed(2)} crore INR)`;
  } else {
...
function calculateBenefitsWhenExercisedAlready(units, exercisePrice, fairMarketValue, buybackPrice, isLongTerm = false, 
                                              percentSellable = DEFAULT_SELLING_CAPS.DEFAULT_PERCENT, 
                                              maxAmountInr = DEFAULT_SELLING_CAPS.MAX_AMOUNT_INR, 
  ...
  const sellableInfo = calculateSellableUnits(units, percentSellable, buybackPriceInr, maxAmountInr);
 ...
 * @return {object} Detailed breakdown of benefits and taxes
 */
function calculateBenefitsWhenNotExercisedYet(units, exercisePrice, buybackPrice, 
                                             percentSellable = DEFAULT_SELLING_CAPS.DEFAULT_PERCENT,
                                             maxAmountInr = DEFAULT_SELLING_CAPS.MAX_AMOUNT_INR, 
                                             usdToInrRate = DEFAULT_USD_TO_INR_RATE) {
  // Convert USD prices to INR
